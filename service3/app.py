...
prompt_template = """Como un Bot de Preguntas Frecuentes para la AUTORIDAD DE SUPERVISION
DEL SISTEMA FINANCIERO, tienes la siguiente informacion sobre una circular emitida por un tribunal:

{context}

Por favor, proporcione la respuesta mas adecuada a la pregunta del usuario.
Respuesta:"""
...
async def service3(conversation_id: str, conversation: Conversation):
    ...
    logger.info(f"Consulta del usuario: {query}")
    logger.info(f"Documentos recuperados: {docs}")

    docs = format_docs(docs=docs)
    logger.info(f"Documentos formateados: {docs}")
